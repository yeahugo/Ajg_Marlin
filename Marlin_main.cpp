...
void get_command_from_wifi()
{
	while( AjgSerial.available() > 0  && buflen < BUFSIZE) {
	   serial_char = AjgSerial.read();
	   if(serial_char == '\n' ||
		  serial_char == '\r' ||
		  ...
		  serial_count >= (MAX_CMD_SIZE - 1) )
	   {
		 ...
		 SERIAL_PROTOCOL("Ajg:");
		 SERIAL_PROTOCOLLN(cmdbuffer[bufindw]);
		 if(!comment_mode){
		   comment_mode = false; //for new command
		   ...
		   if(strchr(cmdbuffer[bufindw], 'N') != NULL)
		   {
			 ...
			 if(gcode_N != gcode_LastN+1 && (strstr_P(cmdbuffer[bufindw], PSTR("M110")) == NULL) ) {
               ...
               //add by zrb
			   AJG_SERIAL_ERROR_START;
			   AJG_SERIAL_ERRORPGM(MSG_ERR_LINE_NO);
			   AJG_SERIAL_ERRORLN(gcode_LastN);
			   FlushAjgSerialRequestResend();
			   serial_count = 0;
			   return;
			 }
			 ...
			 if(strchr(cmdbuffer[bufindw], '*') != NULL)
			 {
			   ...
			   if( (int)(strtod(&cmdbuffer[bufindw][strchr_pointer - cmdbuffer[bufindw] + 1], NULL)) != checksum) {
				 ...
				 AJG_SERIAL_ERROR_START;
				 AJG_SERIAL_ERRORPGM(MSG_ERR_CHECKSUM_MISMATCH);
				 AJG_SERIAL_ERRORLN(gcode_LastN);
				 FlushAjgSerialRequestResend();
				 serial_count = 0;
				 return;
			 ...
			 else
			 {
			   ...
			   AJG_SERIAL_ERROR_START;
			   AJG_SERIAL_ERRORPGM(MSG_ERR_NO_CHECKSUM);
			   AJG_SERIAL_ERRORLN(gcode_LastN);
			   FlushAjgSerialRequestResend();
			   serial_count = 0;
			   return;
		   ...
		   if(strcmp(cmdbuffer[bufindw], "M112") == 0)
			 kill();
			  ...
			  if(card.cardOK){
			  	  if(!card.isFileOpen()){
				  	  card.openFile(MSG_SD_DEF_PRINT_FILE, false);
  			  	  	} 
				  card.write_command(cmdbuffer[bufindw]);
			  	 
			  	}  
		    #endif //SDSUPPORT
        
		   AJG_SERIAL_SUCC_START;
		   AJG_SERIAL_PROTOCOLLN(gcode_LastN+1);

		   SERIAL_PROTOCOL("OK N");
...
void FlushAjgSerialRequestResend()
{
  //char cmdbuffer[bufindr][100]="Resend:";
  AjgSerial.flush();
  AJG_SERIAL_PROTOCOLPGM(AJG_MSG_RESEND);
  AJG_SERIAL_PROTOCOLLN(gcode_LastN + 1);
  AjgClearToSend();
}
